...
# ===== LOCATION PICKER =====
selected_location_id = render_location_picker(
    api.location_cache,
    key_prefix="plant",
    exclude=["Pitch Pile", "Unknown"],
    label="### 📍 Where are you planting?",
)
            ...
            if submitted:
                if selected_unit_id and number_of_units > 0:
                    try:
                        ...
                        if selected_location_id and loc_df is not None:
                            sel = loc_df[loc_df["LocationID"] == selected_location_id]
                            if not sel.empty:
...
with log_col:
    ...
    if planting_df is not None and not planting_df.empty:
        ...
        if not week_plantings.empty:
            st.caption(
                f"**This week:** {len(week_plantings)} plantings, "
                f"{week_plantings['ItemID'].nunique()} unique items"
            )
    else:
